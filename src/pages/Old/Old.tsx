...
export default function Old() {
  ...
  const fetchExchangePrice = async (_currentCoin: string, _currentCurrency: string) => {
    try {
      const data = await fetch(`https://api.binance.com/api/v3/ticker/price?symbol=${_currentCoin}${_currentCurrency}`)
  ...
  const fetchMarket = async (_currentCoin?: string, _currentCurrency?: string) => {
    try {
  ...
  const fetchCoinDetails = async (_currentCoin?: string) => {
    try {
      const currentId = filterCoin(_currentCoin ? _currentCoin : currentCoin)
      const data = await fetch(`https://api.coingecko.com/api/v3/coins/${currentId}`)
      const json = await data.json()
      setCoinDetails(json)
    } catch (err) {
      console.error(err)
  ...
  const fetchHistoryData = async (_currentCoin?: string) => {
    try {
      const currentId = filterCoin(_currentCoin ? _currentCoin : currentCoin)
      const data = await fetch(`https://api.coingecko.com/api/v3/coins/${currentId}/history?date=30-12-2017`)
      const json = await data.json()
      setHistoryData(json)
    } catch (err) {
      console.error(err)
  ...
  const fetchMarketChart = async (_currentCoin?: string, _currentCurrency?: string) => {
    try {
      const currentId = filterCoin(_currentCoin ? _currentCoin : currentCoin)
      let cur
      if (_currentCurrency === "USDT") {
  ...
  const fetchMarketRange = async (_currentCoin?: string, _currentCurrency?: string) => {
    try {
      const currentId = filterCoin(_currentCoin ? _currentCoin : currentCoin)
      let cur
      if (_currentCurrency === "USDT") {
  ...
  const fetchOHLC = async (_currentCoin?: string, _currentCurrency?: string) => {
    try {
      const currentId = filterCoin(_currentCoin ? _currentCoin : currentCoin)
      let cur
      if (_currentCurrency === "USDT") {
      ...
      const data = await fetch(`https://api.coingecko.com/api/v3/coins/${currentId}/ohlc?vs_currency=${cur}&days=1`)
      const json = await data.json()
      setOhlc(json)
    } catch (err) {
      console.error(err)
